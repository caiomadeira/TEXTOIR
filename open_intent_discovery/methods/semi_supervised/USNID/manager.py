...
class USNIDManager:
    ...
    def load_pretrained_model(self, args, pretrained_model):
        ...
        if len(self.data.tokenizer) != self.model.bert.config.vocab_size:
            print(f"Redimensionando embeddings: {self.model.bert.config.vocab_size} -> {len(self.data.tokenizer)}")
            self.model.bert.resize_token_embeddings(len(self.data.tokenizer))
